...
"""
╔══════════════════════════════════════════════════════════════════════════╗
║  🚀 SERVER Zefor — Professional Hosting Control Panel                      ║
...
║  - Modern responsive UI (SERVER Zefor theme)                               ║
╚══════════════════════════════════════════════════════════════════════════╝
"""
...
PROFILE_IMAGE_URL = "https://t.me/sorfelt/3"
ENTRY_SOUND_URL   = "https://t.me/sorfelt/4"
...
init_json_file(OWNER_CONFIG_FILE, {
    ...
    'panel_name': 'SERVER Zefor', 'welcome_msg': 'Welcome to SERVER Zefor'
})
...
# ─────────────────────────────────────────────
#  6b.  OTP Telegram Bot (Registration Verification)
# ─────────────────────────────────────────────
OTP_BOT_TOKEN   = '8717635674:AAEfoouCtG4Pji7UNg0yD_SCSja6tqhW6kI'
OTP_BOT_LINK    = 'http://t.me/OTBAPPBOT'
OTP_DEV_CHANNEL = 'https://t.me/M24Loader'
OTP_DEV_PROFILE = 'https://t.me/Y0YY12'

OTP_CODES_FILE  = os.path.join(BASE_PATH, 'otp_codes.json')
init_json_file(OTP_CODES_FILE, {})

OTP_TTL_SECONDS      = 5 * 60   # code valid for 5 minutes
OTP_RESEND_COOLDOWN  = 45       # seconds between two OTP requests for the same id
OTP_MAX_VERIFY_TRIES = 5        # max wrong attempts before a code is invalidated

def load_otp_codes():
    return load_json_file(OTP_CODES_FILE, {})

def save_otp_codes(d):
    save_json_file(OTP_CODES_FILE, d)

def generate_otp_code():
    """Cryptographically-secure 5 digit numeric code."""
    return ''.join(secrets.choice(string.digits) for _ in range(5))

def tg_api_call(method, payload, timeout=10):
    try:
        r = requests.post(f'https://api.telegram.org/bot{OTP_BOT_TOKEN}/{method}',
                           json=payload, timeout=timeout)
        return r.json()
    except Exception as e:
        return {'ok': False, 'error': str(e)}

def create_registration_otp(telegram_id):
    """Generate + store an OTP for a Telegram ID.
    Returns (code, wait_seconds). code is None if caller must wait
    `wait_seconds` before a new code can be requested."""
    codes = load_otp_codes()
    key = str(telegram_id)
    now = time.time()
    existing = codes.get(key)
    if existing and (now - existing.get('created_at', 0)) < OTP_RESEND_COOLDOWN:
        wait = int(OTP_RESEND_COOLDOWN - (now - existing['created_at']))
    ...
    save_otp_codes(codes)
    # Best effort push — works if the user already opened the bot at least once.
    tg_api_call('sendMessage', {
        'chat_id': telegram_id,
        'text': ('🔑 كود التحقق الخاص بحسابك على SERVER Zefor:\n\n'
                  f'{code}\n\n'
                  '⏳ صالح لمدة 5 دقائق. لا تشاركه مع أي شخص.')
    })
    return code, 0

def verify_registration_otp(telegram_id, code):
    """Check a submitted OTP. Returns (ok: bool, error_message: str)."""
    codes = load_otp_codes()
    key = str(telegram_id)
    rec = codes.get(key)
    if not rec:
        return False, '❌ لم يتم إرسال أي كود لهذا Telegram ID. اضغط زر "إرسال الكود" أولاً.'
    if rec.get('used'):
        return False, '❌ تم استخدام هذا الكود مسبقاً. اطلب كوداً جديداً.'
    if time.time() > rec.get('expires_at', 0):
        codes.pop(key, None)
        save_otp_codes(codes)
        return False, '❌ انتهت صلاحية الكود. اطلب كوداً جديداً.'
    if rec.get('attempts', 0) >= OTP_MAX_VERIFY_TRIES:
        codes.pop(key, None)
        save_otp_codes(codes)
        return False, '❌ تجاوزت عدد المحاولات المسموح. اطلب كوداً جديداً.'
    if not code or rec['code'] != code.strip():
...
def _otp_bot_poller():
    """Long-poll Telegram getUpdates and answer /start + the inline OTP
    button of the registration-verification bot (@OTBAPPBOT)."""
    welcome_text = (
        '👋 مرحباً بك في بوت OTP الخاص بموقع SERVER Zefor!\n\n'
        '🔑 اضغط على زر "OTP" بالأسفل لاستلام آخر كود تحقق تم إرساله إلى حسابك.\n'
        'إذا لم يصدر أي كود حتى الآن سيتم إخبارك بذلك.'
    )
    keyboard = {
        'inline_keyboard': [
            [{'text': '🔑 OTP', 'callback_data': 'get_otp'}],
            [{'text': '📢 قناة المطور', 'url': OTP_DEV_CHANNEL}],
            [{'text': '👨\u200d💻 المطور', 'url': OTP_DEV_PROFILE}],
        ]
    }
    # Make sure no webhook is set, otherwise getUpdates fails with a conflict.
    tg_api_call('deleteWebhook', {'drop_pending_updates': False})
    offset = 0
    while True:
        try:
            resp = requests.get(
                f'https://api.telegram.org/bot{OTP_BOT_TOKEN}/getUpdates',
            ...
            for upd in data.get('result', []):
                ...
                msg = upd.get('message')
                if msg and str(msg.get('text', '')).startswith('/start'):
                    tg_api_call('sendMessage', {
                        'chat_id': msg['chat']['id'],
                        'text': welcome_text,
                ...
                if cb and cb.get('data') == 'get_otp':
                    chat_id = cb['message']['chat']['id']
                    codes = load_otp_codes()
                    rec = codes.get(str(chat_id))
                    if rec and time.time() <= rec.get('expires_at', 0):
                        text = f"🔑 آخر كود تحقق تم إرساله:\n\n{rec['code']}"
                    else:
                        text = '❌ لا يوجد كود حالياً.'
                    tg_api_call('sendMessage', {'chat_id': chat_id, 'text': text})
                    tg_api_call('answerCallbackQuery', {'callback_query_id': cb['id']})
        except Exception:
            time.sleep(5)

if OTP_BOT_TOKEN:
    threading.Thread(target=_otp_bot_poller, daemon=True).start()
...
def admin_required(f):
    """Allow MASTER or any user with role='admin'"""
    @wraps(f)
    def w(*a, **kw):
...
def api_key_required(f):
    ...
    def w(*a, **kw):
        key = (request.headers.get('X-API-Key') or
               ...
               (request.json or {}).get('api_key', '') if request.is_json else '')
        if not key:
            return jsonify({'success':False,'error':'API key required'}), 401
        # check master key
        mcfg = load_master_config()
        if key == mcfg.get('master_api_key',''):
            request.api_user = MASTER_USERNAME
            return f(*a, **kw)
        # check user keys
        users = load_users()
        for uname, ud in users.items():
            if isinstance(ud, dict) and ud.get('api_key') == key and ud.get('active'):
                request.api_user = uname
                return f(*a, **kw)
...
<title>Maintenance — SERVER Zefor</title>
<style>
...
<div class="sub">SERVER Zefor</div>
<div class="msg">{{ message }}</div>
<div class="foot">All rights reserved © SERVER Zefor — By @Y0YY12</div>
</div></body></html>
'''

# ─────────────────────────────────────────────────────────────────────────────
#  18.  AUTH TEMPLATE  (Login + Register — SERVER Zefor theme)
# ─────────────────────────────────────────────────────────────────────────────
AUTH_TEMPLATE = r'''
<!DOCTYPE html>
<html lang="en">
...
<title>SERVER Zefor — Access</title>
<style>
...
.btn-otp{
  padding:12px 14px;border:1px solid rgba(124,92,252,.5);border-radius:8px;
  background:rgba(124,92,252,.12);color:#a78bfa;font-weight:700;font-size:11.5px;
  cursor:pointer;transition:.2s;white-space:nowrap;
}
.btn-otp:hover{background:rgba(124,92,252,.22)}
.btn-otp:disabled{opacity:.5;cursor:not-allowed}
.otp-hint{
  display:block;margin:-6px 0 14px;font-size:11.5px;color:#7c5cfc;
  text-decoration:none;
}
.otp-hint:hover{color:#a78bfa}
.msg{
...
<div class="wrap">
  <div class="card">
    ...
    <div class="logo-block">
      ...
      <div class="logo-title">SERVER Zefor</div>
    ...
    <div class="forms-area">
      ...
      <form class="form" id="register-form" method="post" action="/register">
        ...
        <div class="field"><label>🆔 Telegram ID <span style="color:#f85149">*</span></label><input name="telegram_id" id="reg-tg-id" placeholder="123456789" required autocomplete="off" inputmode="numeric" pattern="[0-9]{5,15}" title="أرقام فقط"></div>
        <a class="otp-hint" href="''' + OTP_BOT_LINK + r'''" target="_blank">📲 افتح بوت OTP لاستلام الكود — @OTBAPPBOT</a>
        <div class="otp-row">
          <div class="field"><label>🔑 OTP Code</label><input name="otp_code" id="reg-otp" placeholder="•••••" required autocomplete="off" inputmode="numeric" maxlength="5" pattern="[0-9]{5}"></div>
          <button type="button" class="btn-otp" id="send-otp-btn" onclick="sendRegOtp()">إرسال الكود</button>
        </div>
        ...
        <div id="otp-status" class="msg" style="display:none"></div>
        {% if error and error_type == 'register' %}
          <div class="msg {% if '✅' in error or 'sent' in error.lower() %}success{% else %}error{% endif %}">{{ error }}</div>
        {% endif %}
      </form>

      <div class="foot">SERVER Zefor &copy; 2025 &nbsp;·&nbsp; By <a href="https://t.me/Y0YY12" target="_blank">@Y0YY12</a></div>
    </div>
...
async function sendRegOtp(){
  const idInput = document.getElementById('reg-tg-id');
  const statusEl = document.getElementById('otp-status');
  const btn = document.getElementById('send-otp-btn');
  const tgId = (idInput.value||'').trim();
  statusEl.style.display='flex';
  if(!/^[0-9]{5,15}$/.test(tgId)){
    statusEl.className='msg error';
    statusEl.textContent='❌ أدخل Telegram ID صحيح (أرقام فقط، 5-15 رقم)';
    return;
  }
  btn.disabled = true;
  ...
  btn.textContent = '...';
  try{
    const r = await fetch('/api/auth/send-otp',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({telegram_id:tgId})});
    ...
    if(d.success){
      ...
      statusEl.className='msg error';
      statusEl.textContent='❌ '+(d.error||'حدث خطأ، حاول مجدداً');
    }
  }catch(e){
    statusEl.className='msg error';
    statusEl.textContent='❌ تعذر الاتصال بالخادم';
  }
  setTimeout(()=>{btn.disabled=false; btn.textContent=origText;}, 30000);
}
(function(){
...
# ─────────────────────────────────────────────────────────────────────────────
#  19.  MAIN DASHBOARD TEMPLATE  (SERVER Zefor)
# ─────────────────────────────────────────────────────────────────────────────
...
<div class="tab-content" id="tab-owner">
  ...
  <div class="section-card">
    ...
    <div class="section-body">
      <div class="field-block"><label>Panel Name</label><input id="panel-name-inp" placeholder="SERVER Zefor"></div>
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>SERVER Zefor — Control Panel</title>
<style>
:root{
...
/* console tab — fixed box, page does NOT scroll */
#tab-console{display:none;flex-direction:column;padding-bottom:16px}
...
/* ── VS Code-style terminal — FIXED height, no scroll on page ── */
.term-window{
...
<div class="topbar">
  <div class="brand">
    <span class="brand-icon">🚀</span> SERVER Zefor
  </div>
  <div class="icons">
    ...
    <div class="section-body">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-top:10px">
        ...
        <a href="https://t.me/Notfound_ELMODMEN" target="_blank" style="text-decoration:none">
          <div class="stat-card"><div class="lbl">📢 Channel</div><div class="val" style="font-size:12px;color:var(--yellow)">@M24Loader</div></div>
        </a>
  ...
  <div class="ai-chat-wrap">
    ...
    <div class="ai-header">
      <div class="ai-header-left">
        ...
        <div>
          <div class="ai-header-title">SERVER Zefor AI</div>
...
<div class="tab-content" id="tab-settings">
  ...
  <!-- API Key Section -->
  <div class="section-card" style="border-color:rgba(124,92,252,.4)">
    <div class="section-head" style="color:#a78bfa">🔑 API Key — مفتاح الـ API</div>
    <div class="section-body">
      <p style="color:var(--text2);font-size:12px;margin-bottom:12px">استخدم هذا المفتاح للوصول إلى الـ API من بوت تيليجرام أو أي تطبيق خارجي.</p>
      <div class="field-block">
        <label>مفتاح الـ API الخاص بك</label>
        <div style="display:flex;gap:8px">
          <input id="my-api-key" type="text" readonly style="flex:1;font-family:monospace;font-size:12px;color:#a78bfa;background:var(--bg2);border:1px solid rgba(124,92,252,.3);border-radius:6px;padding:10px 12px" placeholder="اضغط تحديث لعرض مفتاحك">
          <button class="btn-action" onclick="copyApiKey()">📋 نسخ</button>
          <button class="btn-action danger" onclick="regenerateApiKey()">🔄 تجديد</button>
        </div>
      </div>
      <div style="background:var(--bg2);border:1px solid var(--border);border-radius:8px;padding:14px;margin-top:10px">
        <div style="color:#a78bfa;font-size:12px;font-weight:700;margin-bottom:8px">📖 طريقة الاستخدام:</div>
        <pre style="color:var(--text2);font-size:11px;font-family:monospace;white-space:pre-wrap;line-height:1.7">
# رفع ملف:
...
# بدء بوت تيليجرام:
POST /v1/api/telegram/bot/start
Header: X-API-Key: YOUR_KEY
Body: {"bot_token": "TOKEN", "filename": "bot.py"}

# قراءة ملف:
GET /v1/api/files/content?path=FILE_PATH
Header: X-API-Key: YOUR_KEY</pre>
      </div>
      <div class="row-end" style="margin-top:10px">
        <button class="btn-action gray" onclick="loadApiKey()">🔄 عرض مفتاحي</button>
      </div>
    </div>
  </div>
...
async function powerAction(action){
  if(action==='start'){
    ...
    appendConsole('┌──(𝙴𝙻𝙼𝙾𝙳𝙼𝙴𝙽㉿server-Zefor)-[~]');
...
function terminalBannerHTML(tid){
  return `<div class="t-dim" style="font-size:10px;margin-bottom:4px;user-select:none;padding:2px 0 4px;border-bottom:1px solid #21262d">` +
    ...
    `  <span style="color:#3d444d">SERVER Zefor v2.0 · @Y0YY12 · terminal #${tid}</span></div>` +
    `<div class="t-prompt">┌──(<span style="color:#ff3399">runner</span>㉿<span style="color:#58a6ff">server Zefor</span>)-[<span style="color:#e3b341">~</span>]</div>` +
...
function buildTerminalEl(tid){
  ...
  titlebar.innerHTML =
    ...
    `<div class="term-title">runner㉿server Zefor — terminal ${tid}</div>` +
  ...
  inputRow.innerHTML =
    `<div class="term-prompt-label" id="cwd-prompt-${tid}">` +
      ...
      `<span style="color:#58a6ff">server Zefor</span>` +
  ...
  try{
    ...
    (d.users||[]).forEach(u=>{
      ...
      const roleBadge = isAdmin
        ? `<span style="background:rgba(124,92,252,.2);border:1px solid #7c5cfc;color:#a78bfa;border-radius:4px;padding:1px 8px;font-size:10px;font-weight:700">👑 ADMIN</span>`
        : `<span style="background:rgba(40,40,40,.5);border:1px solid var(--border);color:var(--text3);border-radius:4px;padding:1px 8px;font-size:10px">USER</span>`;
      card.innerHTML=`
        <div class="section-body">
          <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:10px;flex-wrap:wrap">
            <div>
              <div style="font-size:15px;font-weight:700;color:var(--text);display:flex;align-items:center;gap:8px">
                👤 ${escapeHtml(u.username)} ${roleBadge}
              </div>
              <div style="font-size:12px;color:var(--text2);margin-top:4px;display:flex;flex-wrap:wrap;gap:8px">
                ...
                <span style="color:var(--text3);font-size:11px">📋 ${u.max_sessions||1} sessions · ${u.max_servers||1} servers</span>
              </div>
              ...
              ${u.api_key?`<div style="font-size:10px;color:var(--text3);margin-top:4px;font-family:monospace">🗝 API: ${escapeHtml(u.api_key.substring(0,20))}...</div>`:''}
            </div>
            <div style="display:flex;gap:6px;flex-wrap:wrap">
              ...
              <button class="btn-action" style="background:rgba(124,92,252,.2);border:1px solid #7c5cfc;color:#a78bfa" onclick="setUserPermissions('${escapeHtml(u.username)}')">🔧 Permissions</button>
              ${isAdmin
                ? `<button class="btn-action gray" onclick="setUserRole('${escapeHtml(u.username)}','user')">👤 Remove Admin</button>`
                : `<button class="btn-action" style="background:rgba(124,92,252,.15);border:1px solid #7c5cfc;color:#c4b5fd" onclick="setUserRole('${escapeHtml(u.username)}','admin')">👑 Make Admin</button>`
              }
...
async function loadApiKey(){
  ...
  const d=await r.json();
  if(d.api_key){ const el=document.getElementById('my-api-key'); if(el) el.value=d.api_key; }
}

async function copyApiKey(){
  const el=document.getElementById('my-api-key');
  if(!el||!el.value){ toast('اضغط "عرض مفتاحي" أولاً',true); return; }
  try{ await navigator.clipboard.writeText(el.value); toast('✅ تم نسخ المفتاح'); }
  catch(e){ toast('❌ فشل النسخ',true); }
}

async function regenerateApiKey(){
  if(!confirm('تجديد المفتاح سيلغي المفتاح القديم. هل أنت متأكد؟')) return;
  const r=await fetch('/api/user/api-key/regenerate',{method:'POST'});
  const d=await r.json();
  if(d.api_key){
    const el=document.getElementById('my-api-key');
    if(el) el.value=d.api_key;
    toast('✅ تم تجديد المفتاح بنجاح');
  } else { toast('❌ فشل التجديد',true); }
}

// ─── Admin: set user role ───
async function setUserRole(username, role){
  if(!confirm(`تحويل ${username} إلى ${role}؟`)) return;
  const r=await fetch('/api/admin/set-role',{method:'POST',
    headers:{'Content-Type':'application/json'},
    body:JSON.stringify({username,role})});
  const d=await r.json();
  toast(d.success?`✅ ${username} = ${role}`:'❌ '+(d.error||''), !d.success);
  if(d.success) loadUsers();
}

// ─── Admin: set permissions ───
async function setUserPermissions(username){
  const sessions=prompt('Max Sessions:', '1'); if(sessions===null) return;
  const servers=prompt('Max Servers:', '1'); if(servers===null) return;
  const days=prompt('Extend by days (leave blank to skip):', '');
  const body={username, max_sessions:parseInt(sessions), max_servers:parseInt(servers)};
  if(days){ body.expiry=(new Date(Date.now()+parseInt(days)*86400000)).toISOString(); }
  const r=await fetch('/api/admin/user/permissions',{method:'POST',
    headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
  const d=await r.json();
  toast(d.success?'✅ تم تحديث الصلاحيات':'❌ '+(d.error||''), !d.success);
  if(d.success) loadUsers();
}
...
async function loadOwnerPanel(){
  ...
  try{
    ...
    if(pnEl) pnEl.value=ownerCfg.panel_name||'SERVER Zefor';
...
def register_page():
    ...
    if not username or not password or not telegram_id or not otp_code:
        return render_template_string(AUTH_TEMPLATE, error='❌ يرجى ملء جميع الحقول المطلوبة', error_type='register')
    if not re.match(r'^[0-9]{5,15}$', telegram_id):
        return render_template_string(AUTH_TEMPLATE, error='❌ Telegram ID يجب أن يكون أرقاماً فقط (5-15 رقم)', error_type='register')
    if password != confirm:
    ...
    otp_ok, otp_err = verify_registration_otp(telegram_id, otp_code)
    if not otp_ok:
        return render_template_string(AUTH_TEMPLATE, error=otp_err, error_type='register')

    users = load_users()
    if username in users:
        return render_template_string(AUTH_TEMPLATE, error='❌ اسم المستخدم محجوز مسبقاً', error_type='register')

    # 7 days free trial — activated immediately (no admin approval needed)
    expiry_dt = (datetime.now() + timedelta(days=7)).isoformat()
    api_key = 'SH-' + secrets.token_hex(24)   # random API key per user
    users[username] = {
        ...
        'expiry':       expiry_dt,
        'plan':         'free_trial',
        'active':       True,   # activated immediately — no approval required
        'role':         'user',
        'api_key':      api_key,
    }
    ...
    save_users(users)
    ensure_user_folder(username)
    log_activity(username, 'auth.register', f'tg_id={telegram_id} | otp_verified | auto-activated')
    return render_template_string(AUTH_TEMPLATE,
        error=f'✅ تم إنشاء الحساب بنجاح! يمكنك تسجيل الدخول الآن.\nمفتاح API الخاص بك: {api_key}',
        error_type='register')

@app.route('/api/auth/send-otp', methods=['POST'])
def send_registration_otp():
    """Generate a fresh OTP for a Telegram ID during registration and
    deliver it through the @OTBAPPBOT verification bot."""
    d = request.get_json(silent=True) or {}
    telegram_id = str(d.get('telegram_id', '')).strip()
    if not re.match(r'^[0-9]{5,15}$', telegram_id):
        return jsonify({'success': False, 'error': 'Telegram ID غير صالح (أرقام فقط، 5-15 رقم)'})
    code, wait = create_registration_otp(telegram_id)
    if code is None:
        return jsonify({'success': False, 'error': f'يرجى الانتظار {wait} ثانية قبل طلب كود جديد'})
    return jsonify({
        'success': True,
        'message': f'تم إرسال الكود! افتح بوت @OTBAPPBOT واضغط زر "OTP" لاستلامه ({OTP_BOT_LINK})'
    })
...
def get_profile():
    ...
    users = load_users()
    ud = users.get(u, {})
    is_admin = (u == MASTER_USERNAME) or (isinstance(ud,dict) and ud.get('role') == 'admin')
    return jsonify({
        'username': u,
        'is_master': u == MASTER_USERNAME,
        'is_admin': is_admin,
        'role': 'master' if u == MASTER_USERNAME else (ud.get('role','user') if isinstance(ud,dict) else 'user'),
        'api_key': ud.get('api_key','') if isinstance(ud,dict) else '',
        'user_path': p,
...
def admin_set_role():
    d = request.json or {}
    target  = d.get('username','').strip()
    role    = d.get('role','user').strip()    # 'user' | 'admin'
    if not target:
        return jsonify({'success':False,'error':'username required'})
    if role not in ('user','admin'):
        return jsonify({'success':False,'error':'role must be user or admin'})
    if target == MASTER_USERNAME:
        return jsonify({'success':False,'error':'Cannot change master role'})
    users = load_users()
    if target not in users:
        return jsonify({'success':False,'error':'User not found'})
    users[target]['role'] = role
    save_users(users)
    log_activity(session['username'], 'admin.set_role', f'{target}={role}')
    return jsonify({'success':True,'username':target,'role':role})

# ─── API: Admin — list all users (accessible by master OR admin) ───
...
def admin_list_users():
    ...
    for uname, ud in users.items():
        ...
        out.append({
            ...
            'max_sessions': ud.get('max_sessions',1),
            'max_servers':  ud.get('max_servers',1),
            'api_key':      ud.get('api_key',''),
        })
    return jsonify({'users': out})

# ─── API: Admin — approve / suspend user ───
...
def admin_approve_user():
    ...
    save_users(users)
    log_activity(session['username'], 'admin.approve', target)
...
def admin_suspend_user():
    ...
    save_users(users)
    log_activity(session['username'], 'admin.suspend', target)
    return jsonify({'success':True})

# ─── API: Admin — give permissions to user (sessions, servers, plan, expiry) ───
@app.route('/api/admin/user/permissions', methods=['POST'])
@admin_required
def admin_set_permissions():
    ...
    if not target:
        return jsonify({'success':False,'error':'username required'})
    users = load_users()
    if target not in users or not isinstance(users[target], dict):
        return jsonify({'success':False,'error':'User not found'})
    ud = users[target]
    if 'max_sessions' in d:  ud['max_sessions'] = int(d['max_sessions'])
    if 'max_servers'  in d:  ud['max_servers']  = int(d['max_servers'])
    if 'plan'         in d:  ud['plan']          = str(d['plan'])
    if 'expiry'       in d:  ud['expiry']        = str(d['expiry'])
    if 'active'       in d:  ud['active']        = bool(d['active'])
    save_users(users)
    log_activity(session['username'], 'admin.permissions', target)
    return jsonify({'success':True,'username':target,'updated':ud})

# ─── API: User — regenerate own API key ───
@app.route('/api/user/api-key/regenerate', methods=['POST'])
@login_required
def regenerate_api_key():
    u = session['username']
    if u == MASTER_USERNAME:
        # generate/update master API key in config
        new_key = 'SH-' + secrets.token_hex(24)
        MASTER_CONFIG['master_api_key'] = new_key
        save_json_file(MASTER_CONFIG_FILE, MASTER_CONFIG)
        log_activity(u, 'api_key.regenerate', 'master')
        return jsonify({'success':True,'api_key':new_key})
    users = load_users()
    if u not in users: return jsonify({'success':False,'error':'Not found'})
    new_key = 'SH-' + secrets.token_hex(24)
    users[u]['api_key'] = new_key
    save_users(users)
    log_activity(u, 'api_key.regenerate', u)
    return jsonify({'success':True,'api_key':new_key})

# ─── External REST API (authenticated via X-API-Key) ───────────────────────────
@app.route('/v1/api/info')
@api_key_required
def ext_api_info():
    """GET /v1/api/info — returns account info for the API key owner"""
    u = request.api_user
    users = load_users()
    ud = users.get(u, {}) if u != MASTER_USERNAME else {}
    return jsonify({
        'success': True,
        'username': u,
        'role': 'master' if u == MASTER_USERNAME else ud.get('role','user'),
        'plan': ud.get('plan','master') if ud else 'master',
        'expiry': ud.get('expiry','∞') if ud else '∞',
    })

@app.route('/v1/api/files', methods=['GET'])
@api_key_required
def ext_api_list_files():
    """GET /v1/api/files?path=... — list files"""
    u = request.api_user
    p = request.args.get('path', get_user_path(u))
    if not is_path_allowed(u, p):
    ...
    return jsonify({'success':True,'files':files,'path':p})

@app.route('/v1/api/files/content', methods=['GET'])
@api_key_required
def ext_api_get_file():
    """GET /v1/api/files/content?path=... — read file content"""
    u = request.api_user
    p = request.args.get('path','')
    if not p or not is_path_allowed(u, p):
        return jsonify({'success':False,'error':'forbidden'}), 403
    try:
        with open(p,'r',encoding='utf-8',errors='ignore') as f:
            return jsonify({'success':True,'content':f.read(),'path':p})
    except Exception as e:
        return jsonify({'success':False,'error':str(e)}), 500

@app.route('/v1/api/files/upload', methods=['POST'])
@api_key_required
def ext_api_upload_file():
    """POST /v1/api/files/upload — upload a file (multipart/form-data)"""
    u = request.api_user
    f = request.files.get('file')
    p = request.form.get('path', get_user_path(u))
    if not f:
    ...
    if ext in BLOCKED_EXTENSIONS:
        return jsonify({'success':False,'error':f'Extension .{ext} blocked'}), 403
    os.makedirs(p, exist_ok=True)
    ...
    if threats:
        os.remove(sp)
        return jsonify({'success':False,'error':'Security block: '+' | '.join(threats)}), 403
    log_activity(u, 'api.file.upload', filename)
    return jsonify({'success':True,'filename':filename,'path':sp})

@app.route('/v1/api/files/save', methods=['POST'])
@api_key_required
def ext_api_save_file():
    """POST /v1/api/files/save — write file content {path, content}"""
    u  = request.api_user
    d  = request.json or {}
    p  = d.get('path','')
    if not p or not is_path_allowed(u, p):
    ...
    log_activity(u, 'api.file.save', p)
    return jsonify({'success':True,'path':p})

@app.route('/v1/api/exec', methods=['POST'])
@api_key_required
def ext_api_exec():
    """POST /v1/api/exec — run shell command {command, cwd?}"""
    u   = request.api_user
    d   = request.json or {}
    cmd = d.get('command','').strip()
    cwd = d.get('cwd', get_user_path(u))
    if not cmd:
        return jsonify({'success':False,'error':'command required'}), 400
    if not is_path_allowed(u, cwd):
        return jsonify({'success':False,'error':'forbidden cwd'}), 403
    try:
        r = subprocess.run(cmd, shell=True, cwd=cwd,
                           capture_output=True, text=True, timeout=60)
        log_activity(u, 'api.exec', cmd[:80])
        return jsonify({'success':True,'output':r.stdout+r.stderr,'code':r.returncode})
    except subprocess.TimeoutExpired:
        return jsonify({'success':False,'error':'Timeout'}), 408
    except Exception as e:
        return jsonify({'success':False,'error':str(e)}), 500

@app.route('/v1/api/telegram/bot/start', methods=['POST'])
@api_key_required
def ext_api_tg_bot_start():
    """POST /v1/api/telegram/bot/start — start a Telegram bot file
    Body: {bot_token, filename, webhook_url?}
    """
    u = request.api_user
    d = request.json or {}
    bot_token   = d.get('bot_token','').strip()
    filename    = d.get('filename','main.py').strip()
    webhook_url = d.get('webhook_url','').strip()
    if not bot_token:
        return jsonify({'success':False,'error':'bot_token required'}), 400
    user_path = get_user_path(u)
    filepath  = os.path.join(user_path, filename)
    if not os.path.exists(filepath):
        return jsonify({'success':False,'error':f'File not found: {filename}'}), 404
    # inject BOT_TOKEN env var and run
    env = os.environ.copy()
    env['BOT_TOKEN']   = bot_token
    env['WEBHOOK_URL'] = webhook_url
    env['PYTHONUNBUFFERED'] = '1'
    cmd = get_run_command(filepath)
    p   = subprocess.Popen(cmd, shell=True, cwd=user_path,
                           stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                           text=True, bufsize=1, env=env)
    pid = f"{u}_tgbot_{int(time.time())}"
    file_processes[pid] = {'process':p,'filename':filename,'username':u,'output':[]}
    threading.Thread(target=read_process_output, args=(pid,p),
                     kwargs={'store':file_processes}, daemon=True).start()
    log_activity(u, 'api.tg_bot.start', filename)
    return jsonify({'success':True,'process_id':pid,'message':f'Bot started from {filename}'})

@app.route('/v1/api/telegram/bot/output/<pid>', methods=['GET'])
@api_key_required
def ext_api_tg_bot_output(pid):
    """GET /v1/api/telegram/bot/output/<pid> — get bot output"""
    u = request.api_user
    if pid not in file_processes:
        return jsonify({'success':False,'error':'Process not found'}), 404
    info = file_processes[pid]
    if info['username'] != u and u != MASTER_USERNAME:
        return jsonify({'success':False,'error':'Forbidden'}), 403
    out = list(info.get('output',[]))
    info['output'].clear()
    return jsonify({'success':True,'output':out,'is_running':info['process'].poll() is None})

@app.route('/v1/api/telegram/bot/stop', methods=['POST'])
@api_key_required
def ext_api_tg_bot_stop():
    """POST /v1/api/telegram/bot/stop — stop a bot {process_id}"""
    u   = request.api_user
    ...
    if info['username'] != u and u != MASTER_USERNAME:
        return jsonify({'success':False,'error':'Forbidden'}), 403
    try:
        if hasattr(os,'killpg'): os.killpg(os.getpgid(info['process'].pid), signal.SIGKILL)
        else: info['process'].kill()
    except Exception: pass
    del file_processes[pid]
    log_activity(u, 'api.tg_bot.stop', pid)
    return jsonify({'success':True})

@app.route('/v1/api/response', methods=['POST'])
@api_key_required
def ext_api_response():
    """POST /v1/api/response — generic JSON response endpoint (echo/test)"""
    d = request.json or {}
    return jsonify({'success':True,'received':d,'api_user':request.api_user,
                    'timestamp':datetime.now().isoformat()})

# ─── API: get own API key info ───
@app.route('/api/user/api-key')
@login_required
def get_api_key():
    u = session['username']
    if u == MASTER_USERNAME:
        key = MASTER_CONFIG.get('master_api_key','')
        if not key:
            key = 'SH-' + secrets.token_hex(24)
            MASTER_CONFIG['master_api_key'] = key
            save_json_file(MASTER_CONFIG_FILE, MASTER_CONFIG)
    ...
    try:
        ...
        if threats:
            ...
            try:
                ...
                tg_user = ud.get('telegram_id') or ud.get('tg_username', 'غير معروف') if isinstance(ud, dict) else 'غير معروف'
...
|_____|_____|_|  |_|\___/|____/|_|  |_|_____|_| \_|'''+R)
    print(P+'\u2554'+'\u2550'*64+'\u2557'+R)
    print(P+'\u2551  \U0001f680  '+B+C+'\U0001d834\U0001d835\U0001d836\U0001d837\U0001d838\U0001d839\U0001d83a\U0001d83b'+R+P+'  \u2015  SERVER Zefor v2.0  \u2015  @Y0YY12      \u2551'+R)
    print(P+'\u255a'+'\u2550'*64+'\u255d'+R)
    print(G+'\u250c\u2500\u2500('+P+B+'\U0001d834\U0001d835\U0001d836\U0001d837\U0001d838\U0001d839\U0001d83a\U0001d83b'+R+G+'\u1F19A'+C+'server-Zefor'+G+')-['+Y+'~'+G+']'+R)
